...
def create_dates_table(start_date, end_date):
    ...
    # print(date_df.shape)
    date_df = pd.concat(
        [date_df, pd.DataFrame({'validity_date_end_month': pd.date_range(start_date, end_date, freq="M")})], axis=1)
    # print(date_df.shape)
...
def feature_engineering_transactions(df_trans, dates_df):
    ...
    trans_df["count_month_alltrans_account"] = trans_df.groupby(['account_id', 'monthtable_rownumber'])[
        "trans_id"].transform('count')
    ...
    for cat in type_cats:
        ...
        trans_df["count_month_" + cat] = trans_df.groupby(['account_id', 'monthtable_rownumber'])[
            "count_month_" + cat].transform('sum')
        ...
        trans_df["sum_amount_month_" + cat] = trans_df.groupby(['account_id', 'monthtable_rownumber'])[
            "sum_amount_month_" + cat].transform('sum')
        ...
        trans_df["avg_amount_month_" + cat] = trans_df.groupby(['account_id', 'monthtable_rownumber'])[
            "avg_amount_month_" + cat].transform('mean')
    ...
    # balance
    # avg month balance
    trans_df["avg_month_balance"] = trans_df.groupby(['account_id', 'monthtable_rownumber'])["balance"].transform(
        'mean')
    ...
    trans_df = trans_df.sort_values(by=["account_id", "monthtable_rownumber", "trans_id"])
    trans_df["max_acc_month_transid"] = trans_df.groupby(['account_id', 'monthtable_rownumber'])["trans_id"].transform(
        'max')
    trans_df["endmonth_acc_balance"] = np.where(trans_df["max_acc_month_transid"] == trans_df["trans_id"],
                                                trans_df["balance"], np.nan)
    trans_df = trans_df.sort_values(by=["account_id", "monthtable_rownumber", "trans_id"])
    # total month withdrawl/end month balance
    trans_df["ratio_month_wtdrl_endbalance"] = trans_df["sum_amount_month_withdrawal"] / trans_df[
        "endmonth_acc_balance"]
    trans_df["ratio_month_wtdrl_endbalance"] = trans_df["ratio_month_wtdrl_endbalance"].round(2)
    # credit+withdrawl/balance
    trans_df["ratio_month_wtdrl_cred_endbalance"] = (trans_df["sum_amount_month_withdrawal"] + trans_df[
        "avg_amount_month_credit"]) / trans_df["endmonth_acc_balance"]
    ...
    type_operation_dict = {'credit': ['collection from another bank', 'credit in cash'],
                           'withdrawal': [cat for cat in operation_list if
                                          cat not in ['collection from another bank', 'credit in cash']]}
    # print(type_operation_dict)
    # features
    for key in list(type_operation_dict.keys()):
        for var in type_operation_dict[key]:
            trans_df["sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")] = np.where(
                trans_df["operation"] == var, trans_df["amount"], 0)
            trans_df["sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")] = \
            trans_df.groupby(['account_id', 'monthtable_rownumber'])[
                "sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")].transform('sum')
            # I will not work with amounts, only with proportions
            trans_df["pct_sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")] = trans_df[
                                                                                               "sum_amount_month_optype_" + key + "_" + var.replace(
                                                                                                   " ", "_")] / \
                                                                                           trans_df[
                                                                                               "sum_amount_month_" + key]
            trans_df["pct_sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")] = trans_df[
                "pct_sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")].round(2)
            # if we dont have any at the relevant type we will recieve nan. I will impute at 0
            trans_df["pct_sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")] = trans_df[
                "pct_sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")].fillna(0)
            del trans_df["sum_amount_month_optype_" + key + "_" + var.replace(" ", "_")]
    # k_symbol
    k_symbol_list = list(trans_df["k_symbol"].value_counts().index)
    # print(k_symbol_list)
    type_k_symbol_dict = {'credit': ['interest credited', 'old-age pension'],
                          'withdrawal': [cat for cat in k_symbol_list if
                                         cat not in ['interest credited', 'old-age pension']]}
    # print(type_k_symbol_dict)
    # features
    for key in list(type_k_symbol_dict.keys()):
        for var in type_k_symbol_dict[key]:
            trans_df["sum_amount_month_ktype_" + key + "_" + var.replace(" ", "_")] = np.where(
                trans_df["k_symbol"] == var, trans_df["amount"], 0)
            trans_df["sum_amount_month_ktype_" + key + "_" + var.replace(" ", "_")] = \
            trans_df.groupby(['account_id', 'monthtable_rownumber'])[
                "sum_amount_month_ktype_" + key + "_" + var.replace(" ", "_")].transform('sum')
            # I will not work with amounts, only with proportions
            trans_df["pct_sum_amount_month_ktype_" + key + "_" + var.replace(" ", "_")] = trans_df[
                                                                                              "sum_amount_month_ktype_" + key + "_" + var.replace(
                                                                                                  " ", "_")] / trans_df[
                                                                                              "sum_amount_month_" + key]
            trans_df["pct_sum_amount_month_ktype_" + key + "_" + var.replace(" ", "_")] = trans_df[
                "pct_sum_amount_month_ktype_" + key + "_" + var.replace(" ", "_")].round(2)
            # if we dont have any at the relevant type we will recieve nan. I will impute at 0
            trans_df["pct_sum_amount_month_ktype_" + key + "_" + var.replace(" ", "_")] = trans_df[
                "pct_sum_amount_month_ktype_" + key + "_" + var.replace(" ", "_")].fillna(0)
    ...
    vars_to_remove = ["transaction_date", "type", "operation", "amount", "balance", "k_symbol", "bank", "account",
                      "trans_id", "max_acc_month_transid"]
    ...
    cols_for_ma_list = ["count_month_alltrans_account", "count_month_credit", "count_month_withdrawal",
                        "count_month_withdrawal in cash", "sum_amount_month_credit", "sum_amount_month_withdrawal",
                        "sum_amount_month_withdrawal in cash", "ratio_month_wtdrl_credit", "std_month_balance",
                        "min_avg_ratio_month_balance", "endmonth_acc_balance", "ratio_month_wtdrl_endbalance",
                        "ratio_month_wtdrl_cred_endbalance",
                        "pct_sum_amount_month_optype_credit_collection_from_another_bank",
                        "pct_sum_amount_month_optype_credit_credit_in_cash",
                        "pct_sum_amount_month_optype_withdrawal_withdrawal_in_cash",
                        ...
                        "pct_sum_amount_month_ktype_credit_interest_credited",
                        "pct_sum_amount_month_ktype_credit_old-age_pension",
                        "pct_sum_amount_month_ktype_withdrawal_payment_for_statement",
                        "pct_sum_amount_month_ktype_withdrawal_household",
                        "pct_sum_amount_month_ktype_withdrawal_other",
                        "pct_sum_amount_month_ktype_withdrawal_insurrance_payment",
                        "pct_sum_amount_month_ktype_withdrawal_sanction_interest_if_negative_balance"]
    for col in cols_for_ma_list:
        for period in [3, 6]:
            trans_df['ma_month_' + str(period) + "_" + col] = trans_df.groupby(['account_id'])[col].transform(
                lambda x: x.rolling(period, period - 1).mean())
    # ma ratios
    for col in cols_for_ma_list:
...
def clean_and_join_other_data(data):
    ...
    mean_unemp95 = data["district"][data["district"]["unemploymant rate '95"] != "?"]["unemploymant rate '95"].astype(
        float).mean().round(2)
    mean_crime95 = data["district"][data["district"]["no. of commited crimes '95"] != "?"][
        "no. of commited crimes '95"].astype(float).mean().round(0)
    # print(mean_unemp95)
    # print(mean_crime95)
    data["district"].loc[data["district"]["unemploymant rate '95"] == "?", "unemploymant rate '95"] = mean_unemp95
    data["district"].loc[
        data["district"]["no. of commited crimes '95"] == "?", "no. of commited crimes '95"] = mean_crime95
...
def join_data_final(df, trans_df, data):
    ...
    df_joined["client_age"] = (df_joined["validity_date_end_month"].astype('datetime64[ns]') - df[
        'birth_number']) / timedelta(days=365)
    ...
    df_joined["card_type"] = np.where(
        (df_joined["card_issued"] > df_joined["validity_date_end_month"].astype('datetime64[ns]')) | (
            df_joined["card_type"].isna()), "no_card", df_joined["card_type"])
    df_joined["years_since_card_issued"] = np.where(df_joined["card_type"] == "no_card", -100.0, (
                df_joined["validity_date_end_month"].astype('datetime64[ns]') - df_joined['card_issued']) / timedelta(
        days=365))
    ...
    df_joined["years_since_card_issued"] = df_joined.groupby(['account_id', 'monthtable_rownumber'])[
        "years_since_card_issued"].transform('max')
    # no need for account date
    df_joined["years_since_account_date"] = (df_joined["validity_date_end_month"].astype('datetime64[ns]') - df_joined[
        'account_date']) / timedelta(days=365)
    ...
    df_joined["unemploymant rate '95"] = np.where(
        np.datetime64('1995-01-01') <= df_joined["validity_date_end_month"].astype('datetime64[ns]'),
        df_joined["unemploymant rate '95"], -100.0)
    df_joined["unemploymant rate '96"] = np.where(
        np.datetime64('1996-01-01') <= df_joined["validity_date_end_month"].astype('datetime64[ns]'),
        df_joined["unemploymant rate '96"], -100.0)
    df_joined["no. of commited crimes '95"] = np.where(
        np.datetime64('1995-01-01') <= df_joined["validity_date_end_month"].astype('datetime64[ns]'),
        df_joined["no. of commited crimes '95"], -100.0)
    df_joined["no. of commited crimes '96"] = np.where(
        np.datetime64('1996-01-01') <= df_joined["validity_date_end_month"].astype('datetime64[ns]'),
        df_joined["no. of commited crimes '96"], -100.0)
    df_joined["pctchg_unempl_9695"] = np.where(
        np.datetime64('1996-01-01') <= df_joined["validity_date_end_month"].astype('datetime64[ns]'),
        df_joined["pctchg_unempl_9695"], -100.0)
    df_joined["pctchg_crime_9695"] = np.where(
        np.datetime64('1996-01-01') <= df_joined["validity_date_end_month"].astype('datetime64[ns]'),
        df_joined["pctchg_crime_9695"], -100.0)
    accounts_with_cards = \
    df_joined[df_joined["card_type"] != "no_card"].sort_values(by=["account_id", "monthtable_rownumber", "card_type"])[
        "account_id"].tolist()
    # bring it to account level
    # gender will be owner gender
    df_joined = df_joined[df_joined["type"] == "OWNER"]
    # print(df_joined.shape)
    vars_to_remove_joined = ["client_id", "disp_id", "type", "birth_number", "code", "card_issued", "client_age",
                             "account_date"]
    ...
    df_joined["target_loan"] = np.where(
        (df_joined["loan_date"] > df_joined["validity_date_end_month"].astype('datetime64[ns]'))
        | (df_joined["card_type"].isna()), 0, df_joined["target_loan"])
    df_joined["target_amount"] = np.where(
        (df_joined["loan_date"] > df_joined["validity_date_end_month"].astype('datetime64[ns]'))
...
def features_pipeline(index, X_train, y_train, X_test, y_test, columns, row, spark, key=None, date=None, static_cols=[],
    ...
    timeseries = TimeSeriesTransformer(key=key, date=date, key_col=X_train.reset_index()[key],
                                       date_col=X_train.reset_index()[date],
