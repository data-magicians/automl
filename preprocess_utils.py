...
def feature_engineering_transactions(df_trans, dates_df):
    ...
    type_operation_dict = {'credit':['collection from another bank','credit in cash'],
                          'withdrawal':[cat for cat in operation_list if cat not in ['collection from another bank','credit in cash']]}
    ...
    type_k_symbol_dict = {'credit':['interest credited','old-age pension'],
                          'withdrawal':[cat for cat in k_symbol_list if cat not in ['interest credited','old-age pension']]}
    ...
    cols_for_ma_list = ["count_month_alltrans_account","count_month_credit","count_month_withdrawal","count_month_withdrawal in cash",
                   ...
                   "ratio_month_wtdrl_credit",
                   "std_month_balance","min_avg_ratio_month_balance","endmonth_acc_balance",
                    "ratio_month_wtdrl_endbalance","ratio_month_wtdrl_cred_endbalance",
...
def join_data_final(df,trans_df,data):
    ...
    df_joined["card_type"] = np.where( (df_joined["card_issued"]>df_joined["validity_date_end_month"].astype('datetime64[ns]'))
                                     | (df_joined["card_type"].isna()), "no_card",df_joined["card_type"])

    df_joined["years_since_card_issued"] = np.where(df_joined["card_type"]=="no_card",
                                       -100.0,
                                       (df_joined["validity_date_end_month"].astype('datetime64[ns]') - df_joined['card_issued'])/timedelta(days=365))
    ...
    np.datetime64('1995-01-01')<=df_joined["validity_date_end_month"].astype('datetime64[ns]'),
    df_joined["no. of commited crimes '95"],
    -100.0
    )

    df_joined["no. of commited crimes '96"] = np.where(
    np.datetime64('1996-01-01')<=df_joined["validity_date_end_month"].astype('datetime64[ns]'),
    df_joined["no. of commited crimes '96"],
    -100.0
    )
    ...
    np.datetime64('1996-01-01')<=df_joined["validity_date_end_month"].astype('datetime64[ns]'),
    df_joined["pctchg_unempl_9695"],
    -100.0
    )

    df_joined["pctchg_crime_9695"] = np.where(
    np.datetime64('1996-01-01')<=df_joined["validity_date_end_month"].astype('datetime64[ns]'),
    df_joined["pctchg_crime_9695"],
    -100.0
    )
